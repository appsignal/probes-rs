...
#[cfg(target_os = "linux")]
mod test {
    use super::os::{read_and_parse_v1_sys_stat, read_and_parse_v2_sys_stat};
    use super::{CgroupCpuMeasurement, CgroupCpuStat};
    use crate::error::ProbeError;
    use std::path::Path;
    ...
    fn test_read_v1_sys_measurement() {
        let measurement =
            read_and_parse_v1_sys_stat(&Path::new("fixtures/linux/sys/fs/cgroup/cpuacct_1/"))
                .unwrap();
    ...
    fn test_read_v2_sys_measurement() {
        let measurement =
            read_and_parse_v2_sys_stat(&Path::new("fixtures/linux/sys/fs/cgroup/cpu.stat_1"))
                .unwrap();
        let cpu = measurement.stat;
    ...
    fn test_read_and_parse_v2_sys_stat_incomplete() {
        match read_and_parse_v2_sys_stat(&Path::new(
            "fixtures/linux/sys/fs/cgroup/cpu.stat_incomplete",
        )) {
            Err(ProbeError::UnexpectedContent(_)) => (),
            r => panic!("Unexpected result: {:?}", r),
        }
    ...
    fn test_read_and_parse_v2_sys_stat_garbage() {
        let path = Path::new("fixtures/linux/sys/fs/cgroup/cpu.stat_garbage");
    ...
    fn test_in_percentages_integration_v1() {
        ...
        let mut measurement2 =
            read_and_parse_v1_sys_stat(&Path::new("fixtures/linux/sys/fs/cgroup/cpuacct_2/"))
                .unwrap();
    ...
    fn test_in_percentages_integration_v2() {
        ...
        measurement1.precise_time_ns = 375953965125920;
        let mut measurement2 =
            read_and_parse_v2_sys_stat(&Path::new("fixtures/linux/sys/fs/cgroup/cpu.stat_2"))
                .unwrap();
        measurement2.precise_time_ns = 376013815302920;
